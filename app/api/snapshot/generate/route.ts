...
async function upsertSnapshotForMatch(
  ...
  try {
    ...
    // ‰ΩøÁî® match.match_date ‰ΩúÁÇ∫?≤Â??∫Ê?Ôºõvenue Áº∫Â§±??travelKm=0ÔºåË≥ΩÁ®ãÂ?Â∫¶‰?Ê≠?∏∏Ë®àÁ?
    const current_venue = match.home_team?.home_city || "Unknown";
    const feature_vector = await buildFeatureVector(
      baseFakeFeatures,
      match.home_team_id,
      match.away_team_id,
      match.match_date,  // Ê≠∑Âè≤?∫Ê?Èª?      current_venue
    );
...
export async function POST(request: Request) {
  try {
    ...
    try { body = await request.json(); } catch { /* Á©?body = ?πÈ?Ê®°Â? */ }
    ...
    // Ë®∫Êñ∑Â±?    const [total_matches, matches_without_any_snapshot] = await Promise.all([
      prisma.matches.count(),
      prisma.matches.count({ where: { snapshots: { none: {} } } }),
    ]);

    // rebuild=trueÔºöÊ??®ÈÉ® matchesÔºàÂº∑?∂Ë??ãË?Âø´ÁÖßÔº?    // rebuild=falseÔºöÂè™?ÉÈ?Ê≤íÊ?Âø´ÁÖß??matches
    ...
    return NextResponse.json({
      ...
      updated_count: updated,   // rebuild Ê®°Â?‰∏ãÈÄôÂÄãÊï∏Â≠óÊ?Ë©≤Á?Ë°?      error_count: errors,
