...
export default function MatchupGauge({ m, adjustedOverride, v11 }: Props) {
  ...
  const factors: { label: string; magnitude: number; dir: '+' | '-'; detail: string }[] = [
    ...
    },
    {
      ...
      magnitude: 0.026,
      dir: '+',
      detail: `Player State gap: ${Math.round(m.recovery_away * 100)}% vs ${Math.round(m.recovery_home * 100)}% // biggest gap on today's slate`,
    },
  ]
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: 16, padding: '8px 0 20px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          <RingGauge value={baseline} size={170} thickness={10} color="#60a5fa" track="#0b1220" label="BASELINE" sublabel="GAME STATE" />
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
          <div style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 9, color: '#475569', letterSpacing: '0.3em', fontWeight: 800 }}>
            TEAM STATE SHIFT
          </div>
          <div
            style={{
          ...
          <div style={{ fontSize: 16, color: edgeColor, fontWeight: 800 }}>{positive ? 'UP' : 'DOWN'}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
          <RingGauge
            ...
            color="#22d3ee"
            track="#0b1220"
            label={v11 ? 'COACH MODE' : 'TEAM STATE'}
            sublabel="MOSPORT CORE"
          />
        </div>
      </div>
      ...
      >
        ...
        {v11 ? (
          ...
          <span style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 10, color: '#94a3b8', letterSpacing: '0.1em' }}>
            Coach Mode favors <span style={{ color: '#fff', fontWeight: 800 }}>{m.perspective}</span> control / {m.away.abbr}
          </span>
        )}
      </div>
